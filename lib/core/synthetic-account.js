...
class SyntheticAccountService {
    ...
    async create(payload) {
        ...
        if(payload.accountNumber) {
            newRequest['account_number'] = payload.accountNumber;
        }

        if(payload.routingNumber) {
            newRequest['routing_number'] = payload.routingNumber;
        }

        if(payload.plaidProcessorToken) {
            newRequest['plaid_processor_token'] = payload.plaidProcessorToken;
        }
        ...
        if (validator.isEmpty(payload.poolUid, { ignore_whitespace: true })) {
            throw new Error('Create Synthetic Account "poolUid" is required.');
        }

        if (validator.isEmpty(payload.syntheticAccountTypeUid, { ignore_whitespace: true })) {
            throw new Error('Create Synthetic Account "syntheticAccountTypeUid" is required.');
        }

        if (payload.accountNumber) {
            if(!utils.isString(payload.accountNumber)) {
                throw new Error('Create Synthetic Account "account_number" must be a string.');
            }
        }

        if (payload.routingNumber) {
            if (!utils.isString(payload.routingNumber)) {
                throw new Error('Create Synthetic Account "routing_number" must be a string.');
            }
        }

        if (payload.plaidProcessorToken) {
            if (!utils.isString(payload.plaidProcessorToken)) {
                throw new Error('Create Synthetic Account "plaidProcessorToken" must be a string.');
            }
        }
