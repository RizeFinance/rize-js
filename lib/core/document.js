...
class DocumentService {
    ...
    async getList(query = {}) {
        ...
        const response = await this._api.get(`/documents/${queryStr}`);
     ...
     * @returns {Promise<HTTPResponse<string>>} A promise that returns a binary Document with headers if resolved.
    ...
    async viewBase64(uid, extension = 'pdf') {
        const response = await this.view(uid, extension);
    ...
    async get(uid) {
        ...
        const response = await this._api.get(`/documents/${uid}`);
