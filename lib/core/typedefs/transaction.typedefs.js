 ...
 * @property {string} uid - A unique identifier generated by Rize
 * @property {number} settled_index - A unique index that indicates the order in which Transaction Events were settled
 * @property {string} transaction_uid - UID of the Transaction this line item belongs to
 * @property {number} phase - Some Transfer types will trigger multiple transfers at the custodial level. Some of these custodial transfers must wait for all dependent custodial transfers to complete before initiating. Transaction Events with `phase` value 1 are recorded for the first set of custodial transfers that get initiated after the (Synthetic) Transfer starts. Custodial transfers in later phases cannot initiate before all events from previous phases are settled.
 * @property {string} source_custodial_account_uid - Custodial Account from which the asset is pulled
 * @property {string} destination_custodial_account_uid - Custodial Account where the asset lands
 * @property {Array<string>} custodial_line_item_uids - A list of UIDS referring to Custodial Line Items belonging to this event. There are always at least two line items if the status is `settled`.
 * @property {string} status
 * @property {string} us_dollar_amount - The amount will never be negative
 * @property {'odfi_ach_deposit'|'odfi_ach_withdrawal'|'rdfi_ach_deposit'|'rdfi_ach_withdrawal'} type
 * @property {'positive'|'negative'|'neutral'} net_asset - Indicates whether the Customer's asset has gone up (`positive`), gone down (`negative`) or stayed the same (`neutral`) as a result of this Transaction Event. This value is determined by `type`.
 * @property {string} description
 * @property {string} created_at - The date and time when the event is first recognized by Rize. This may be when the Transfer is initiated if the event is associated with one.
 * @property {string} settled_at - The date and time when the event is settled.
 */

/**
 * @typedef {Object} TransactionEventListQuery
 * @property {Array<string>} [source_custodial_account_uid] - Filter by source Custodial Account.
 * @property {Array<string>} [destination_custodial_account_uid] - Filter by destination Custodial Account.
 * @property {Array<string>} [custodial_account_uid] - Filter by Custodial Account, source or destination.
 * @property {Array<'odfi_ach_deposit'|'odfi_ach_withdrawal'|'rdfi_ach_deposit'|'rdfi_ach_withdrawal'>} [type] - Filter by type. Multiple values are allowed.
 * @property {Array<string>} [transaction_uid] - Filter by Transaction. Multiple values are allowed
 * @property {number} [limit] - Maximum number of items to retrieve. This filter is automatically applied with the default value if not given. Default = 100.
 * @property {number} [offset] - Index of the items to start retrieving from. Default = 0.
 * @property {'created_at_asc'|'created_at_desc'|'description_asc'|'description_desc'|'settled_index_asc'|'settled_index_desc'|'us_dollar_amount_asc'|'us_dollar_amount_desc'} [sort]
 */

/**
 * @typedef {Object} SyntheticLineItem
 * @property {string} uid - A unique identifier generated by Rize
 * @property {number} settled_index - A unique index that indicates the order in which the Synthetic Line Items were settled
 * @property {string} transaction_uid - UID of the Transaction this Synthetic Line Item belongs to
 * @property {string} synthetic_account_uid - UID of the Synthetic Account this Synthetic Line Item is associated with
 * @property {string} status
 * @property {string} us_dollar_amount - The signed USD amount of this specific Synthetic Line Item
 * @property {string} running_us_dollar_balance - The Synthetic Account balance after this Synthetic Line Item was settled
 * @property {string} description - Human-readable description in context of Synthetic Accounts
 * @property {string} created_at - The date and time when the Synthetic Line Item is first recognized by Rize. This may be when the Transfer is initiated if the Synthetic Line Item is associated with one.
 * @property {string} settled_at - The date and time when the Synthetic Line Item is settled
 */

/**
 * @typedef {Object} SyntheticLineItemListQuery
 * @property {Array<string>} [customer_uid] - Filter by Customer.
 * @property {Array<string>} [pool_uid] - Filter by pool.
 * @property {Array<string>} [synthetic_account_uid] - Filter by Synthetic Account.
 * @property {number} [limit] - Maximum number of items to retrieve. This filter is automatically applied with the default value if not given. Default = 100.
 * @property {number} [offset] - Index of the items to start retrieving from. Default = 0.
 * @property {Array<string>} [transaction_uid] - Filter by Transaction.
 * @property {Array<'begun'|'failed'|'in_progress'|'settled'>} [status] - Filter by status.
 * @property {created_at_asc'|'created_at_desc'|'description_asc'|'description_desc'|'settled_index_asc'|'settled_index_desc'|'us_dollar_amount_asc'|'us_dollar_amount_desc} [sort]
 */

/**
 * @typedef {Object} CustodialLineItem
 * @property {string} uid - A unique identifier generated by Rize.
 * @property {number} settled_index - A unique index that indicates the order in which the Custodial Line Items were settled.
 * @property {string} transaction_uid - UID of the Transaction this Custodial Line Item belongs to.
 * @property {string} transaction_event_uid - UID of the Transaction Event this Custodial Line Item belongs to.
 * @property {string} custodial_account_uid - UID of the Custodial Account this Custodial Line Item is associated with.
 * @property {string} status
 * @property {string} us_dollar_amount - The signed USD amount of this specific Custodial Line Item
 * @property {string|null} [running_us_dollar_balance] - US dollar balance of the Custodial Account as of the completion of this Custodial Line Item. This field may be empty unless the status is `settled` or `voided`.
 * @property {string} type - Symbols and/or numbers that represent different types of Custodial Line Items.
 * @property {string} description - Descriptions pulled from the custodians, such as through NACHA description or ISO-8583 data element 43.
 * @property {string} created_at - The date and time when the Custodial Line Items is first recognized by Rize. This may be when the Transfer is initiated if the Custodial Line Item is associated with one.
 * @property {string} occurred_at - This field may be empty if status is not settled or voided.
 * @property {string} settled_at - The date and time when the Custodial Line Item is settled.
 */

/**
 * @typedef {Object} CustodialLineItemListQuery
 * @property {Array<string>} [customer_uid] - Filter by Customer.
 * @property {Array<string>} [custodial_account_uid] - Filter by Custodial Account.
 * @property {Array<'settled'|'voided'>} [status] - Filter by status.
 * @property {number} [us_dollar_amount_tax] - Return any records having a us_dollar_amount at or below us_dollar_amount_max.
 * @property {number} [us_dollar_amount_min] - Return any records having a us_dollar_amount at or above us_dollar_amount_min.
 * @property {Array<string>} [transaction_event_uid] - Filter by Transaction Event.
 * @property {Array<string>} [transaction_uid] - Filter by Transaction.
 * @property {number} [limit] - Maximum number of items to retrieve. This filter is automatically applied with the default value if not given. Default = 100.
 * @property {number} [offset] - Index of the items to start retrieving from. Default = 0.
 * @property {'created_at_asc'|'created_at_desc'|'description_asc'|'description_desc'|'settled_index_asc'|'settled_index_desc'|'us_dollar_amount_asc'|'us_dollar_amount_desc'} [sort]
