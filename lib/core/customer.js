...
class CustomerService {
   ...
   */
    _validateGetListQuery(query) {
        ...
        const customer_types = ['primary', 'sole_proprietor', 'secondary'];
   ...
   */
    ...
    _validateCreateParams(email, customer_type) {
        ...
        if (
            customer_type &&
      !['primary', 'sole_proprietor'].includes(customer_type)
        ) {
   ...
   */
    _validateDetailParams(details) {
        ...
        validator.isEmpty(details.phone, { ignore_whitespace: true })
            ...
            if (details.ssn) {
                if (validator.isEmpty(details.ssn, { ignore_whitespace: true })) {
                    throw new Error('"details.ssn" should not be empty.');
                }
            ...
            if (details.dob) {
                if (validator.isEmpty(details.dob, { ignore_whitespace: true })) {
                    throw new Error('"details.dob" should not be empty.');
                }
   ...
   * @param {string} externalUid - A Customer identifier supplied by the Partner, unique among the collection of all partner Customers - optional.
   * @param {string} email - Email of the customer
   * @param {'primary'|'sole_proprietor'} customer_type - Type of customer
   * @returns {Promise<Customer>} - A promise that returns a Customer if resolved.
   * @example const newCustomer = await rize.customer.post(extenalUid, email, customer_type);
   */
    async create(externalUid = null, email, customer_type = 'primary') {
        this._validateCreateParams(email, customer_type);
   ...
   *
   * Creates a new instance of a secondary customer.
   * @param {string} externalUid - A Customer identifier supplied by the Client, unique among the collection of all Client Customers - optional
   * @param {string} primary_customer_uid - The UID of the Primary Customer with whom this Secondary Customer will be affiliated
   * @param {string} email - Email of the Secondary Customer - optional
   * @param {CustomerDetails} details - An object containing the supplied identifying information for the Customer
   * @returns {Promise<Customer>} - A promise that returns a Customer if resolved.
   * @example const newSecondaryCustomer = await rize.customer.createSecondary.post(extenalUid, primary_customer_uid, details);
   */
   ...
   * );
   */
    async update(uid, email = null, details = null, customer_type) {
