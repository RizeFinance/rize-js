...
class CustomerService {
   ...
   * @param {'primary' | 'sole_proprietor' | 'secondary'} customer_type
   * @param {string} primary_customer_uid

   */
    _validateUpdateParams(uid, email, details, customer_type = 'primary', primary_customer_uid) {
        if (validator.isEmpty(uid, { ignore_whitespace: true })) {
            throw new Error('Customer "uid" is required.');
        }
        if (email && !validator.isEmail(email)) {
        ...
        if (customer_type === 'secondary' && validator.isEmpty(primary_customer_uid, { ignore_whitespace: true })) {
            throw new Error('Customer "primary_customer_uid" is required.');
        }
        if (!utils.isObject(details)) {
            throw new Error('"details" should be a CustomerDetails object.');
        } else {
            this._validateDetailParams(details, customer_type);
        }
    }
   ...
   */
    _validateDetailParams(details, customer_type = 'primary') {
        if (!utils.isObject(details)) {
            throw new Error('"details" should be a CustomerDetails object.');
        } else {
            if (
   ...
   */
    async create(externalUid, email, customer_type = 'primary') {
   ...
   */
    async createSecondary(externalUid = null, primary_customer_uid, email = null, details) {
        this._validateUpdateParams(externalUid, email, details, 'secondary', primary_customer_uid);
   ...
   * );
   */
    async update(uid, email, details, customer_type) {
        this._validateUpdateParams(uid, email, details, customer_type);
