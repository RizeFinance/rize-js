    ...
    constructor(api, auth) {
        ...
        this._validateGetListQuery = (query) => {
            ...
            if (!isObject(query)) {
                ...
                if ('first_name' in query && validator.isEmpty(query.first_name, { ignore_whitespace: true })) {
                    throw new Error('"first_name" query is invalid.');
                }
                if ('last_name' in query && validator.isEmpty(query.last_name, { ignore_whitespace: true })) {
                    throw new Error('"last_name" query is invalid.');
                }
                if ('email' in query && !validator.isEmail(query.email)) {
                ...
                if ('program_uid' in query && validator.isEmpty(query.program_uid, { ignore_whitespace: true })) {
                    throw new Error('"program_uid" query is invalid.');
                }
                if ('external_uid' in query && validator.isEmpty(query.external_uid, { ignore_whitespace: true })) {
    ...
    async getList (query={}) {
        ...
        for (const key in query) {
            if (queryParameters.includes(key)) {
                if (key === 'pool_uid') {
                    query[key].forEach(poolUid => {
                        queryStr += `${key}=${poolUid}&`;
                    });
                } else {
