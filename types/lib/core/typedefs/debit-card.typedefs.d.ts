export type DebitCard = {
    ...
    pool_uid: string;
    /**
     * - A UID referring to the specific Synthetic Account to which this Card belongs. Any debit or credit transactions from this Card will occur in this account.
     */
    synthetic_account_uid: string;
    /**
     * - A UID referring to the specific Custodial Account to which this Card belongs. Any debit or credit transactions from this Card will occur in this account.
     */
    custodial_account_uid: string;
    /**
     * - Last 4 digits of the Debit Card number to aid in indentifying the Card. Null if the Card is not yet issued (i.e. status is `initiated`).
     */
     ...
     */
    status: 'queued' | 'issued' | 'printing_physical_card' | 'printing_physical_card_replacement' | 'shipped' | 'card_replacement_shipped' | 'usable_without_pin' | 'normal' | 'closed' | 'damaged' | 'lost' | 'stolen' | 'administrative_lock' | 'closed_by_administrator' | 'shipment_returned' | 'card_replacement_shipment_returned';
    /**
     * - If the Card can be used, this value will be `true`; `false` otherwise.
     */
    ready_to_use: boolean;
    /**
     * - If the Card is in a locked state, this field will be populated describing why the Card was locked. Null if the Card is not locked.
     */
    lock_reason?: string | null;
    /**
     * - The date this Card was issued. It may be briefly null when a new Card is requested, but the issuance has not yet happened (i.e. status is `initiated`).
     */
    issued_on?: string | null;
    /**
     * - The date and time at which this Card was locked. Null if the Card is not locked.
     */
    locked_at?: string | null;
    /**
     * - The date and time at which this Card was closed. This field will be null unless the `status` is `closed`, `closed_by_administrator`, `lost`, or `stolen`.
     */
     ...
     */
    latest_shipping_address?: Address | null;
};
export type DebitCardListQuery = {
    /**
    ...
    /**
     * - Only return locked Cards if `true` and only return unlocked Cards if `false`
     */
    locked?: boolean;
    /**
     * - Filter by status. More values to come. Multiple values are allowed
     */
