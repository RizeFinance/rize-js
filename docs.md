...
-   [CustomerService][13]
    ...
    -   [unlock][31]
        -   [Parameters][32]
        -   [Examples][33]
-   [ComplianceWorkflowSummary][34]
    -   [Properties][35]
-   [ComplianceWorkflowCustomer][36]
    -   [Properties][37]
-   [ComplianceDocument][38]
    -   [Properties][39]
-   [ComplianceDocumentAcknowledgementRequest][40]
    -   [Properties][41]
-   [ComplianceWorkflow][42]
    -   [Properties][43]
...
-   `customerExternalUid` **[string][61]** A Customer identifier supplied by the Partner, unique among the collection of all partner Customers.
-   `email` **[string][61]** Email address associated with the Customer.
...
Returns **[Promise][62]&lt;[ComplianceWorkflow][63]>** A promise that returns the new Compliance Workflow if resolved.
...
-   `customerExternalUid` **[string][61]** A Customer identifier supplied by the Partner, unique among the collection of all partner Customers.
-   `customerUid` **[string][61]** A UID referring to the Customer generated by Rize
-   `email` **[string][61]** Email address associated with the Customer.

Returns **[Promise][62]&lt;[ComplianceWorkflow][63]>** A promise that returns the new Compliance Workflow entity if resolved.
...
-   `customerUid` **[string][61]** A UID referring to the Customer
...
Returns **[Promise][62]&lt;[ComplianceWorkflow][63]>** A promise that returns the latest Compliance Workflow if resolved.
...
-   `complianceWorkflowUid` **[string][61]** A UID referring to the Compliance Workflow.
-   `customerUid` **[string][61]** A UID referring to the Customer.
-   `documents` **...[ComplianceDocumentAcknowledgementRequest][64]** 
...
Returns **[Promise][62]&lt;[ComplianceWorkflow][63]>** A promise that returns the updated Compliance Workflow if resolved.
...
-   `query` **[CustomerListQuery][65]** An object containing key value pair for filtering the results list. (optional, default `{}`)
...
Returns **[Promise][62]&lt;[CustomerList][66]>** A promise that returns a Customer List if resolved.
...
-   `uid` **[string][61]** Rize-generated unique customer id
...
Returns **[Promise][62]&lt;[Customer][67]>** A promise that returns a Customer if resolved.
...
-   `uid` **[string][61]** Rize-generated unique customer id
-   `email` **[string][61]** Email of the customer
-   `details` **[CustomerDetails][68]** An object containing the supplied identifying information for the Customer
...
Returns **[Promise][62]&lt;[Customer][67]>** A promise that returns the updated Customer if resolved.
...
Returns **[Promise][62]&lt;void>** A promise that returns void if resolved.
...
-   `uid` **[string][61]** Rize-generated unique customer id
-   `lockReason` **[string][61]** The reason that the Customer is being locked must be submitted with the request body.
...
Returns **[Promise][62]&lt;[Customer][67]>** A promise that returns the locked Customer if resolved.
...
-   `uid` **[string][61]** Rize-generated unique customer id
-   `unlockReason` **[string][61]** The reason that the Customer is being unlocked. (optional, default `null`)
...
Returns **[Promise][62]&lt;[Customer][67]>** A promise that returns the unlocked Customer if resolved.
...
-   `accepted_quantity` **[number][70]** 
-   `begun_at` **[string][61]** The DateTime at which this Compliance Workflow was requested
-   `completed_step` **[number][70]** 
-   `current_step` **[number][70]** 
...
-   `email` **[string][61]** 
-   `external_uid` **[string][61]** A Customer identifier supplied by the Client, unique among the collection of all Client Customers
-   `uid` **[string][61]** A UID referring to the Customer
...
-   `external_storage_name` **[string][61]** Amazon S3 key used to retrieve the contents of a Compliance Document
-   `compliance_document_url` **[string][61]** Amazon S3 URL used to retrieve the contents of a Compliance Document
-   `name` **[string][61]** 
-   `step` **[number][70]** Multiple Compliance Documents are grouped into a Step, and Compliance Documents are presented to a Customer, Step-by-Step
-   `version` **[number][70]** 
-   `uid` **[string][61]** A UID referring to a Compliance Document; note that this UID will be different for each Customer
-   `accepted_at` **[string][61]** The DateTime at which this Compliance Document was acknowledged
...
-   `documentUid` **[string][61]** A UID referring to the Compliance Document being acknowledged.
-   `accept` **(`"yes"` \| `"no"`)** An indication of acceptance or rejection.
-   `userName` **[string][61]?** A label associated with the Customer (required for electronic signing).
-   `ipAddress` **[string][61]?** A numerical label assigned to each device connected to a computer network that uses the Internet Protocol for communication (required for electronic signing); in this case, the label associated with the computer used by the Customer.
...
-   `uid` **[string][61]** A unique identifier generated by Rize.
-   `summary` **[ComplianceWorkflowSummary][71]** 
-   `customer` **[ComplianceWorkflowCustomer][72]** 
-   `accepted_documents` **[Array][73]&lt;[ComplianceDocument][74]>** 
-   `current_step_documents_pending` **[Array][73]&lt;Omit&lt;[ComplianceDocument][74], `"accepted_at"`>>** Compliance Documents that await acknowledgment in the current Step
-   `all_documents` **[Array][73]&lt;Omit&lt;[ComplianceDocument][74], (`"accepted_at"` \| `"uid"`)>>** The set of all Compliance Documents that would require acknowledgment
...
-   `first_name` **[string][61]?** Only return Customers with a first name matching exactly what is submitted
-   `last_name` **[string][61]?** Only return Customers with a last name matching exactly what is submitted
-   `email` **[string][61]?** Only return Customers with an email address matching exactly what is submitted
-   `locked` **[boolean][75]?** Only return locked Customers if true and only return unlocked Customers if false
-   `program_uid` **[string][61]?** Only return Customers belonging to the submitted Program.
-   `external_uid` **[string][61]?** A unique, immutable id provided by Client.
-   `pool_uid` **[Array][73]&lt;[string][61]>?** Filter by pool. Multiple values are allowed.
-   `limit` **[string][61]?**  Maximum number of items to retrieve. This filter is automatically applied with the default value if not given. Default: 100
-   `offset` **[string][61]?** Index of the items to start retrieving from. Default: 0
...
-   `total_count` **[number][70]** Total count of items available to retrieve
-   `count` **[number][70]** Number of items retrieved
-   `limit` **[number][70]** Maximum number of items to retrieve
-   `offset` **[number][70]** Index of the first item to retrieve
-   `data` **[Array][73]&lt;[Customer][67]>** 
...
-   `uid` **[string][61]** A unique identifier generated by Rize
-   `external_uid` **[string][61]** A unique identifier Client supplies. It should be given when creating a new resource and must be unique within the resource type. If the same value is given, no new resource will be created.
-   `program_uid` **[string][61]** A uid referring to the program this customer belongs to.
-   `pool_uids` **[Array][73]&lt;[string][61]>** A list of uids referring to Pools belonging to this Customer.
-   `email` **[string][61]** Email of the customer
...
-   `total_balance` **[string][61]** Total asset owned by the customer in US dollars.
-   `created_at` **[Date][76]** 
-   `locked_at` **([Date][76] | null)?** The date and time when the Customer was locked. This field will be null if and only if the lock_reason is null.
-   `lock_reason` **([string][61] | null)?** The lock reason provided by the Client, an admin User, or the system at the time the Customer was locked. This field will be null if and only if the locked_at is null.
-   `details` **[CustomerDetails][68]** An object containing the supplied identifying information for the Customer.
...
-   `first_name` **[string][61]** 
-   `middle_name` **([string][61] | null)?** 
-   `last_name` **[string][61]** 
-   `suffix` **([string][61] | null)?** 
-   `phone` **[string][61]** 
-   `ssn` **[string][61]** 
-   `dob` **[string][61]** 
-   `address` **[Address][77]** 
...
-   `timeout` **[number][70]?** Specifies the number of milliseconds before the each request times out. (Default: 80000)
...
-   `programUid` **[string][61]** The Rize Program ID.
-   `hmac` **[string][61]** The HMAC that will be used to sign the JSON web signature in order to get access to the API.
-   `options` **[RizeOptions][78]?** Configuration options (optional, default `RizeOptions`)
...
Type: [ComplianceWorkflowService][79]
...
[34]: #complianceworkflowsummary

[35]: #properties

[36]: #complianceworkflowcustomer

[37]: #properties-1

[38]: #compliancedocument

[39]: #properties-2

[40]: #compliancedocumentacknowledgementrequest

[41]: #properties-3

[42]: #complianceworkflow
