...
```javascript
const transaction = await rize.transaction.getTransactionEvent(transactionUid);
```
...
-   `phase` **integer** Some Transfer types will trigger multiple transfers at the custodial level. Some of these custodial transfers must wait for all dependent custodial transfers to complete before initiating.
...
-   `status` **[string][137]** Other statuses such as pending will be available later
-   `us_dollar_amount` **[string][137]** The amount will never be negative
...
-   `transaction_uid` **[string][137]?** Filter by Transaction. Multiple values are allowed
-   `limit` **[number][157]?** Maximum number of items to retrieve. This filter is automatically applied with the default value if not given. Default = 100.
-   `offset` **[number][157]?** Index of the items to start retrieving from. Default = 0.
