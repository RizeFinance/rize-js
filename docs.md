...
-   `customerExternalUid` **[string][60]** A Customer identifier supplied by the Partner, unique among the collection of all partner Customers.
-   `email` **[string][60]** Email address associated with the Customer.
...
Returns **[Promise][61]&lt;[ComplianceWorkflow][62]>** A promise that returns the new Compliance Workflow if resolved.
...
-   `customerExternalUid` **[string][60]** A Customer identifier supplied by the Partner, unique among the collection of all partner Customers.
-   `customerUid` **[string][60]** A UID referring to the Customer generated by Rize
-   `email` **[string][60]** Email address associated with the Customer.

Returns **[Promise][61]&lt;[ComplianceWorkflow][62]>** A promise that returns the new Compliance Workflow entity if resolved.
...
-   `customerUid` **[string][60]** A UID referring to the Customer
...
Returns **[Promise][61]&lt;[ComplianceWorkflow][62]>** A promise that returns the latest Compliance Workflow if resolved.
...
-   `complianceWorkflowUid` **[string][60]** A UID referring to the Compliance Workflow.
-   `customerUid` **[string][60]** A UID referring to the Customer.
-   `documents` **...[ComplianceDocumentAcknowledgementRequest][63]** 
...
Returns **[Promise][61]&lt;[ComplianceWorkflow][62]>** A promise that returns the updated Compliance Workflow if resolved.
...
-   `query` **[CustomerListQuery][64]** An object containing key value pair for filtering the results list. (optional, default `{}`)
...
Returns **[Promise][61]&lt;[CustomerList][65]>** A promise that returns a Customer List if resolved.
...
-   `uid` **[string][60]** Rize-generated unique customer id
...
Returns **[Promise][61]&lt;[Customer][66]>** A promise that returns a Customer if resolved.
...
-   `uid` **[string][60]** Rize-generated unique customer id
-   `email` **[string][60]** Email of the customer
-   `details` **[CustomerDetails][67]** An object containing the supplied identifying information for the Customer
...
Returns **[Promise][61]&lt;[Customer][66]>** A promise that returns the updated Customer if resolved.
...
Returns **[Promise][61]&lt;void>** A promise that returns void if resolved.
...
-   `begun_at` **[string][60]** The DateTime at which this Compliance Workflow was requested
...
-   `email` **[string][60]** 
-   `external_uid` **[string][60]** A Customer identifier supplied by the Client, unique among the collection of all Client Customers
-   `uid` **[string][60]** A UID referring to the Customer
...
-   `external_storage_name` **[string][60]** Amazon S3 key used to retrieve the contents of a Compliance Document
-   `compliance_document_url` **[string][60]** Amazon S3 URL used to retrieve the contents of a Compliance Document
-   `name` **[string][60]** 
...
-   `version` **[number][69]** 
-   `uid` **[string][60]** A UID referring to a Compliance Document; note that this UID will be different for each Customer
-   `accepted_at` **[string][60]** The DateTime at which this Compliance Document was acknowledged
...
-   `documentUid` **[string][60]** A UID referring to the Compliance Document being acknowledged.
-   `accept` **(`"yes"` \| `"no"`)** An indication of acceptance or rejection.
-   `userName` **[string][60]?** A label associated with the Customer (required for electronic signing).
-   `ipAddress` **[string][60]?** A numerical label assigned to each device connected to a computer network that uses the Internet Protocol for communication (required for electronic signing); in this case, the label associated with the computer used by the Customer.
...
-   `uid` **[string][60]** A unique identifier generated by Rize.
-   `summary` **[ComplianceWorkflowSummary][70]** 
-   `customer` **[ComplianceWorkflowCustomer][71]** 
-   `accepted_documents` **[Array][72]&lt;[ComplianceDocument][73]>** 
-   `current_step_documents_pending` **[Array][72]&lt;Omit&lt;[ComplianceDocument][73], `"accepted_at"`>>** Compliance Documents that await acknowledgment in the current Step
-   `all_documents` **[Array][72]&lt;Omit&lt;[ComplianceDocument][73], (`"accepted_at"` \| `"uid"`)>>** The set of all Compliance Documents that would require acknowledgment
...
-   `middle_name` **([string][60] | null)?** 
-   `last_name` **[string][60]** 
-   `suffix` **([string][60] | null)?** 
-   `phone` **[string][60]** 
-   `ssn` **[string][60]** 
-   `dob` **[string][60]** 
-   `address` **[Address][74]** 
...
-   `uid` **[string][60]** A unique identifier generated by Rize
-   `external_uid` **[string][60]** A unique identifier Client supplies. It should be given when creating a new resource and must be unique within the resource type. If the same value is given, no new resource will be created.
-   `program_uid` **[string][60]** A uid referring to the program this customer belongs to.
-   `pool_uids` **[Array][72]&lt;[string][60]>** A list of uids referring to Pools belonging to this Customer.
-   `email` **[string][60]** Email of the customer
...
-   `total_balance` **[string][60]** Total asset owned by the customer in US dollars.
-   `created_at` **[Date][75]** 
-   `locked_at` **([Date][75] | null)?** The date and time when the Customer was locked. This field will be null if and only if the lock_reason is null.
-   `lock_reason` **([string][60] | null)?** The lock reason provided by the Client, an admin User, or the system at the time the Customer was locked. This field will be null if and only if the locked_at is null.
-   `details` **[CustomerDetails][67]** An object containing the supplied identifying information for the Customer.
...
-   `first_name` **[string][60]?** Only return Customers with a first name matching exactly what is submitted
-   `last_name` **[string][60]?** Only return Customers with a last name matching exactly what is submitted
-   `email` **[string][60]?** Only return Customers with an email address matching exactly what is submitted
-   `locked` **[boolean][76]?** Only return locked Customers if true and only return unlocked Customers if false
-   `program_uid` **[string][60]?** Only return Customers belonging to the submitted Program.
-   `external_uid` **[string][60]?** A unique, immutable id provided by Client.
-   `pool_uid` **[Array][72]&lt;[string][60]>?** Filter by pool. Multiple values are allowed.
-   `limit` **[string][60]?**  Maximum number of items to retrieve. This filter is automatically applied with the default value if not given. Default: 100
-   `offset` **[string][60]?** Index of the items to start retrieving from. Default: 0
...
-   `programUid` **[string][60]** The Rize Program ID.
-   `hmac` **[string][60]** The HMAC that will be used to sign the JSON web signature in order to get access to the API.
-   `options` **[RizeOptions][77]?** Configuration options (optional, default `RizeOptions`)
...
Type: [ComplianceWorkflowService][78]
...
[63]: #compliancedocumentacknowledgementrequest
